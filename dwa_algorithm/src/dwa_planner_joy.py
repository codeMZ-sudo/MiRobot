...
class DWAPlannerMecanum:
    ...
    def __init__(self):
        ...
        #Command for joystick
        self.no_joy_cmd = False
    ...
    def calculate_dynamic_window(self):
        ...
        """
        # Velocity space based on robot max velocities
    ...
    def calculate_obstacle_cost(self, trajectory):
        ...
        Returns high cost if trajectory collides with obstacles,
        otherwise returns inverse of minimum distance.
    ...
    def calculate_heading_cost(self, trajectory):
        ...
        dx = self.goal_pose[0] - final_pose[0]
        dy = self.goal_pose[1] - final_pose[1]
        angle_to_goal = np.arctan2(dy, dx)
        
        # Calculate heading error
        heading_error = abs(np.arctan2(np.sin(angle_to_goal - final_pose[2]),
                                       np.cos(angle_to_goal - final_pose[2])))
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_goal_reached() or self.no_joy_cmd:
                if self.is_goal_reached() and not self.no_joy_cmd: rospy.loginfo("Goal reached!") 
                if self.no_joy_cmd: rospy.loginfo("Esperando comando del joystick")
                self.publish_cmd_vel(0.0, 0.0, 0.0)
